...
const AddProduct = () => {
  ...
  return (
    <div>
        <Navbar/>
        {loggeduser && loggeduser[0].email == "25amruta@gmail.com" ?
        <div className={styles.addprod__container}>
            <form className={styles.addprod__form} onSubmit ={handleAddProduct}>
            <p>Add Data</p>
            {successMsg && <div className={styles.success__msg}>{successMsg}</div>}
            {uploadError && <div className={styles.error__msg}>{uploadError}</div>}
        ...
        </div>: <div>You Don't have access to the products</div>}
